...
const PomodoroTimer: React.FC<PomodoroTimerProps> = ({ task, onComplete, onStop }) => {
    ...
    return (
        <div className="fixed inset-0 bg-slate-100/80 dark:bg-black/80 backdrop-blur-xl flex flex-col items-center justify-center p-4 z-50 grainy-background">
            <div className="w-full max-w-md bg-white/80 dark:bg-black/50 backdrop-blur-2xl rounded-3xl p-8 text-center shadow-2xl border border-slate-200 dark:border-cyan-500/30 dark:shadow-[0_8px_32px_0_rgba(0,0,0,0.37)] dark:shadow-cyan-500/30">
                ...
                <div className="my-10">
                    <p className="text-7xl sm:text-9xl font-mono font-thin tracking-tighter text-slate-800 dark:text-cyan-100 dark:drop-shadow-[0_0_20px_rgba(34,211,238,0.5)]">{formatTime(timeLeft)}</p>
                </div>
                <div className="flex justify-center items-center space-x-6">
                    <button onClick={onStop} className="p-4 rounded-full bg-slate-200/80 dark:bg-black/40 dark:border dark:border-red-500/30 hover:bg-red-500/20 dark:hover:bg-red-500/20 dark:hover:border-red-500/50 text-red-500 dark:text-red-400 hover:text-red-600 dark:hover:text-red-300 transition-all duration-300 transform hover:scale-110">
                        <StopIcon className="w-8 h-8"/>
                    </button>
                     <button onClick={() => setIsActive(!isActive)} className="p-6 rounded-full bg-gradient-to-br from-indigo-500 to-cyan-500 text-white transition-transform hover:scale-110 duration-300 shadow-lg hover:shadow-cyan-400/40 dark:shadow-cyan-500/50 dark:hover:shadow-cyan-500/70">
                        {isActive ? <PauseIcon className="w-10 h-10"/> : <PlayIcon className="w-10 h-10"/>}
                    </button>
                    <button onClick={nextMode} className="p-4 rounded-full bg-slate-200/80 dark:bg-black/40 dark:border dark:border-cyan-500/30 hover:bg-slate-300/80 dark:hover:bg-black/60 dark:hover:border-cyan-500/50 text-slate-600 dark:text-cyan-400 hover:text-slate-800 dark:hover:text-cyan-300 transition-all duration-300 transform hover:scale-110" title="Skip to next session">
                        <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M13 5l7 7-7 7M5 5l7 7-7 7" /></svg>
                    </button>
                </div>
            </div>
        </div>
